...
// InstanceKlass是Java Class 的VM级别表示。
...
protected:
    ...
    enum {
        ...
        1 << 5  // class/superclass/implemented interfaces has default methods
    };
    u2 _misc_flags;
...
public:
    ...
    bool is_override(methodHandle super_method, Handle targetclassloader, Symbol *targetclassname,
                     TRAPS);
    ...
    static bool is_same_class_package(oop class_loader1, Symbol *class_name1, oop class_loader2,
                                      Symbol *class_name2);
    ...
    static ByteSize reference_type_offset() {
        return in_ByteSize(offset_of(InstanceKlass, _reference_type));
    }
    ...
    Method *lookup_method_in_all_interfaces(Symbol *name, Symbol *signature,
    ...
    static ByteSize init_thread_offset() {
        return in_ByteSize(offset_of(InstanceKlass, _init_thread));
    }
    ...
    static int vtable_length_offset() {
        return offset_of(InstanceKlass, _vtable_len) / HeapWordSize;
    }

    intptr_t *start_of_vtable() const {
        return ((intptr_t * )
        this) +vtable_start_offset();
    }
    ...
    int itable_offset_in_words() const {
        return start_of_itable() - (intptr_t * )
        this;
    }
...
private:
    ...
    static void
    set_initialization_state_and_notify_impl(instanceKlassHandle this_oop, ClassState state, TRAPS);
    ...
    static void
    do_local_static_fields_impl(instanceKlassHandle this_oop, void f(fieldDescriptor *fd, TRAPS),
                                TRAPS);
...
public:
    ...
    int decrement() {
        _count -= 1;
        assert(_count >= 0, "don't underflow");
        return _count;
    }
    ...
    InnerClassesIterator(instanceKlassHandle k) {
        ...
        if (k->inner_classes() != NULL) {
            ...
            assert((_length % InstanceKlass::inner_class_next_offset == 0 ||
                    _length % InstanceKlass::inner_class_next_offset ==
                    InstanceKlass::enclosing_method_attribute_size),
                   "just checking");
            // Remove the enclosing_method portion if exists.
            if (_length % InstanceKlass::inner_class_next_offset ==
                InstanceKlass::enclosing_method_attribute_size) {
